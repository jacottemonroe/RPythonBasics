...
> **Question 1**: What is the class of the sum 3+5 and of 3.0+5 in R and in Python? Write a sum of two numbers that returns an integer in R and in Python. 
```
...
```{block, type="alert alert-success"}
> **Question 5**: In R and Python, slice `LocMat` to get an array containing only the values `5.7` and `53`. Hint: You can combine conditions by using `&` (AND) or `|` (OR). 
```
